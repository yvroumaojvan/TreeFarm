...
- **PHP 危险文件上传（CWE-434）**：`move_uploaded_file` 目标/文件名含 `$_FILES` 或变量拼接、
  MIME-only 校验——全部检出（severity high）；文件内有**扩展名白名单校验**（pathinfo+in_array
  各自任意行）自动豁免，零误报
- **PHP 单行标签形态修复（血泪根因）**：`<?php ... ?>` 同一行时 SQLi 漏检——根因是占位符
  判断用了「行内是否含 ?」，`<?php`/`?>` 里的 `?` 被误当 SQL 占位符而豁免！
  改为**占位符形态正则**（`= ?` / `(?,` / `, ?` 才算），标签字符再也不背锅
- **CI 超时加固**：workflow `timeout-minutes: 15`（防答题时长抖动假 FAILED）
- 新增 5 用例（上传 3 变体 + 白名单负例 + 单行形态）；全量 1026 → **1031 全绿**
