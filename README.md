...
├── tree-farm/                  # ★ 技能 2：树场 v4.4.1（Agent Skills 标准格式）
│   ├── SKILL.md                #   两级状态机 + 树场工作流（v4.4.1 同步）
│   └── scripts/
│       ├── tree_farm.py        #   树场引擎 v4.4.1 入口（纯 Python 标准库，SQLite 存储）
│       ├── treefarm/           #   ★ 多模块包（common/parser/storage/config/analysis/core/cli/sandbox）
│       ├── tests/              #   239 个 unittest 测试（python3 -m unittest discover -s tests）
...
| 树场独立包 | `tree-farm/` 技能并入，**升级为 v4.4.1**（函数级分析 + 真增量更新 + SQLite 存储 + 5 大代码分析 + 沙箱动态测试 + 静态AST + 中文命令菜单 + 239 个测试） |
...
| 工程化 | **239 个 unittest 测试全绿**（+ 14 个 Node 树搜索测试）、GitHub Actions CI、--debug 日志、--brief --compact 精简输出 |
