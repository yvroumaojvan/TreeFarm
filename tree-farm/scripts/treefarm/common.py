...
VERSION = "4.9.19"               # 工具版本（v4.9.19：50 轮 R1-R10——语义型 bug 启发式攻下 tornado#2/#4（TE chunked/Range 负偏移，修复版豁免）；CVE 变体 7 新规则（ghp_/JWT/AKIA 密钥/format 注入/shelve/条件表达式 SSRF/多行 f-string）；Java+5（MyBatis ${}/StringBuilder/FileChannel/setAttribute/WebView 桥）；JS+5（execFile -e/间接 eval/got/clone/常量豁免）；跨文件引擎升级（四层链 range(4)/request/req/r 缩写/容器内联源）；性能多重拼接与推导式查找；通用委托不一致检测+is None/True/False 豁免；878 测试全绿）
