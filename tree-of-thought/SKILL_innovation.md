...
description: 灵感树（TreeOfThought 创新模式 v1.1，乖宝 2026-09-22 原创）——让 AI 学会「灵光一现」。内核→表达形式→新颖评分→灵感果实库；v1.1 新增双树桥接（抓虫树剪枝弱信号→灵感种子）。默认关闭、绝不自动触发。开启方式：用户说「灵感树」/「创新模式」/「开创新分支」→ 本次会话启用；用「抓虫树」/「关闭灵感树」切回。与抓虫树共用思维树开关，双模式由 tree.js 统一入口切换。
---

# 💡 灵感树 v1.1（思维树创新模式 · 乖宝原创）
