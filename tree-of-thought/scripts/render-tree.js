 ...
 *
 * v3 支持两种格式：
 *   1. 原版 rounds 格式（SKILL.md 数据模型，见下方注释）
 *   2. tree-search.js 状态格式（nodes/order + status/score）：
 *      渲染带状态徽章（✅ 评分 / ✂️ 剪枝 / 💀 死路 / ⏳ 待展开）的搜索树，
 *      展示评分明细与回溯/剪枝痕迹 —— 与 tree-search.js 配合完成
 *      「真正的树搜索」可视化。
 *
...
function render(data, compact) {
  // 兼容 tree-search.js 状态格式（nodes/order）：渲染带状态徽章的搜索树
  if (data.nodes && data.order) {
    return renderSearchState(data, compact);
  }
  var rounds = getRounds(data);
  var branchCount = 0;
...
/* ================ tree-search.js 状态渲染（v3 新增） ================ */

function searchBadge(n) {
  if (n.status === 'dead_end') return '💀 死路' + (n.dead_reason ? ':' + n.dead_reason : '');
  if (n.status === 'pruned') return '✂️ 剪枝';
  if (n.status === 'expanded') return '🔍 已展开';
  if (n.status === 'scored' && n.score) return '✅ 评分 ' + (n.score.total !== undefined ? n.score.total : n.score);
  return '⏳ 待展开';
}

function renderSearchState(data, compact) {
  var nodes = data.nodes;
  var order = data.order || [];
  var counts = { pruned: 0, dead: 0, scored: 0 };
  order.forEach(function (id) {
    var n = nodes[id];
    if (n.pruned) counts.pruned++;
    if (n.status === 'dead_end') counts.dead++;
    if (n.status === 'scored') counts.scored++;
  });
  var header = MARK.trunk.trim() + ' 思维树 · ' + String(data.strategy || 'beam').toUpperCase()
    + ' 搜索 · ' + order.length + ' 节点'
    + (counts.scored ? ' · ' + counts.scored + ' 已评分' : '')
    + (counts.pruned ? ' · ' + counts.pruned + ' 剪枝' : '')
    + (counts.dead ? ' · ' + counts.dead + ' 死路' : '');
  var lines = [header, '│'];
  var rootId = order[0];
  if (rootId) renderSearchNode(lines, nodes, rootId, '', true, compact);
  return lines.join('\n');
}

function searchNodeText(n) {
  var c = n.content;
  if (typeof c === 'string') return c;
  return (c && (c.conclusion || c.dimension || c.hypothesis)) || '';
}

function renderSearchNode(lines, nodes, id, prefix, isLast, compact) {
  var n = nodes[id];
  var c = n.content || {};
  var label;
  if (n.type === 'trunk') {
    label = MARK.trunk + '树干：' + oneLine(searchNodeText(n), 40);
  } else if (n.type === 'consensus') {
    label = MARK.consensus + '共识：' + oneLine(searchNodeText(n), 50);
  } else {
    label = MARK.branch + (c.dimension || '分支') + '  [' + searchBadge(n) + ']';
    if (!compact && n.score && n.score.evidence !== undefined) {
      var sc = n.score;
      label += '\n' + prefix + (isLast ? '    ' : '│   ') + '    📊 证据' + sc.evidence
        + ' 相关' + sc.relevance + ' 新颖' + sc.novelty + ' 可验证' + sc.verifiable;
    }
    if (!compact && c.conclusion) {
      label += '\n' + prefix + (isLast ? '    ' : '│   ') + '    → ' + oneLine(c.conclusion, 60);
    }
  }
  ...
  var kids = n.children || [];
  for (var i = 0; i < kids.length; i++) {
    renderSearchNode(lines, nodes, kids[i], childPrefix, i === kids.length - 1, compact);
  }
}
